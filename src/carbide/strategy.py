...
COMPANY = 'Carson'
REPO = 'carbide'
PROJECT_TERMS = ['carson', 'whole', 'pitch', 'generative', 'artifact', 'usecarson', 'prospect', 'editor']
PROJECT_METRICS = ['carson_coverage', 'whole_risk', 'pitch_precision', 'generative_latency']
PROJECT_FAILURES = ['carson_drift', 'whole_gap', 'pitch_misroute', 'generative_blindspot']
PROJECT_ARCHETYPES = [{'name': 'carson evidence replay', 'trigger': 'carson signal changes while whole context is stale', 'expected': 'block release until cited evidence is regenerated'}, {'name': 'whole boundary probe', 'trigger': 'whole handoff crosses a policy or trust boundary', 'expected': 'route to reviewer with evidence packet'}, {'name': 'pitch regression harness', 'trigger': 'pitch behavior regresses against the last accepted fixture', 'expected': 'open a regression issue with trace and benchmark delta'}, {'name': 'generative operator packet', 'trigger': 'generative output needs a human-readable audit packet', 'expected': 'accept only if decision claims cite fixture evidence'}]
PROJECT_DIRECTION = 'A typed, auditable generative UI runtime for agentic apps — every rendered component carries a capability scope, every re render is a stable diff, and every action is logged with a cryptographic intent trail.'
...
HERO_TITLE = 'Carson Adversarial Replay Board'
